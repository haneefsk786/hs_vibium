...
The MCP recording path now produces the same Playwright-compatible trace format as the proxy path. Previously, the MCP `browser_record_start` / `browser_record_stop` tools used a separate, simpler recorder — now both paths share the same `proxy.Recorder` and produce identical output.
